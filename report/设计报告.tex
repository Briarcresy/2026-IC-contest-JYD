...
本CPU顶层模块为\code{myCPU}。

源文件位于企业工程的\code{digital\_twin.srcs/sources\_1/imports/news/}目录下。该模块通过系统时钟复位、IROM指令存储器接口以及DRAM/外设统一接口接入企业提供的工程模板。
...
\begin{itemize}
	\item 李建昊：CPU顶层架构设计，Datapath模块设计，流水线寄存器设计，数据前递与冒险处理机制设计，报告撰写，git协作环境搭建，项目管理，设计、仿真与上板测试环境搭建；负责团队成员协调。
	\item 施云翀：参与流水线代码初期实现、初期调试，参与流水线后期优化；完成Trace仿真测试接口设计、兼容性设计；性能测试设计和执行；参与FPGA bitstream生成与上板测试；负责测评报告的整理与撰写。
	\item 袁源：检查各模块源文件的引用路径；整理 CPU 各模块的输入输出端口信息，并制作模块接口说明表；协助准备指令存储器初始化文件，完成基础行为仿真和部分关键信号观察；参与 FPGA bitstream 生成与上板测试，记录测试结果；负责设计报告的整理与撰写。
\end{itemize}
