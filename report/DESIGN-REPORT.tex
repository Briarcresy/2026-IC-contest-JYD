...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{p{3cm}p{2cm}p{2cm}X}
		...
		\code{perip\_addr}  & output & 32 & DRAM或外设访问地址，再由MEM阶段ALU结果给出     \\
		\code{perip\_wen}   & output & 1  & 使能信号，store指令有效                 \\
...
虽然本设计已经完成了基于 RV32I 指令集的五级流水线 CPU 基本功能，并实现了数据前递、load-use 暂停和控制冒险冲刷等机制，但当前设计仍存在以下不足。

首先，当前 CPU 主要面向初赛要求的 RV32I 37 条基础指令进行设计，尚未完整支持 CSR 指令、异常处理、trap 机制以及 RV32M 乘除法扩展。因此，在后续分赛区决赛或更复杂的软件运行场景中，仍需要进一步扩展控制器、数据通路和系统寄存器相关模块。

其次，当前流水线采用较为基础的冒险处理策略。对于 load-use 冒险，本设计通过暂停一个周期保证数据正确；对于分支和跳转指令，本设计在确认跳转发生后冲刷错误路径指令。该方法逻辑清晰、实现可靠，但在分支密集或 load-use 相关较多的程序中会引入一定流水线停顿，影响整体执行效率。

最后，当前 CPU 的性能优化仍以功能正确性为优先。虽然设计中采用了五级流水线和数据前递机制，但对关键路径、资源利用率、最高工作频率和 CPI 等性能指标的分析仍不够深入。后续仍需要结合 Vivado 综合实现报告，对 ALU、前递选择、冒险检测和写回路径等关键逻辑进行进一步时序分析和优化。

\subsection{后续改进方向}

后续工作可以从以下几个方面继续完善。

第一，进一步完善功能。在 RV32I 基础指令稳定通过的基础上，加入浮点数计算等更多特色功能的设计。

第二，优化流水线性能。针对 load-use 冒险和分支控制冒险，可以进一步研究更高效的处理策略，例如优化 load 数据前递路径、提前分支判断阶段，或增加简单的分支预测机制，以减少流水线冲刷和暂停带来的性能损失。

第三，加强时序和资源优化。后续可以根据 Vivado 的综合与实现报告，分析关键路径所在模块，对 ALU 运算逻辑、前递选择逻辑、冒险检测逻辑和写回选择逻辑进行优化。同时，可以比较不同 CPU 时钟频率下的资源利用率和时序裕量，确定更稳定的运行频率。

第四，完善验证体系。除企业提供的测评程序外，还可以增加自定义汇编测试、边界条件测试和随机指令测试。例如，针对跳转边界、符号扩展、无符号比较、字节/半字访存、连续数据相关等情况设计专门测试用例，以提高 CPU 验证的覆盖率。

第五，增强工程可维护性。后续可以进一步规范模块命名、信号命名和注释说明，将控制信号、流水线寄存器信号和调试信号整理成更清晰的接口文档，方便后续团队成员继续扩展和调试。

综上，本设计已经完成了初赛阶段五级流水线 RV32I CPU 的核心功能，但仍有进一步扩展和优化空间。后续将以提高指令集完整性、增强流水线性能、完善异常处理机制和提升验证覆盖率为主要方向，使 CPU 设计更加完整、稳定和可扩展。
