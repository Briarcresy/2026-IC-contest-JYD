...
待补充内容 & \todo{整体架构图截图、Vivado版本、开发板型号、综合实现结果、bit文件信息} \\
\bottomrule
\end{tabularx}
\end{table}
...
项目 & 内容 \\
\midrule
硬件平台 & \todo{填写FPGA开发板型号，例如竞业达数字孪生平台/具体板卡型号} \\
FPGA器件 & \todo{填写FPGA型号，例如Xilinx Kintex-7或实际型号} \\
开发工具 & \todo{填写Vivado版本} \\
硬件描述语言 & SystemVerilog \\
仿真工具 & \todo{填写Vivado Simulation、Verilator或ModelSim等} \\
CPU时钟 & 企业模板默认提供50MHz CPU时钟，可通过PLL调整；本设计使用频率为\todo{填写实际频率} \\
工程顶层 & 企业模板顶层 + 自研\code{myCPU}核心 \\
生成文件 & \todo{填写bit文件名称与生成路径} \\
\bottomrule
\end{tabularx}
\end{table}
...
初赛不测指令 & \code{fence, ecall, ebreak} & \todo{可实现为空指令或后续扩展} \\
\bottomrule
\end{longtable}
...
\begin{tikzpicture}[
    node distance=1.0cm,
    stage/.style={draw, rounded corners, minimum width=2.1cm, minimum height=1.0cm, align=center},
    reg/.style={draw, fill=gray!10, minimum width=0.7cm, minimum height=1.0cm, align=center},
    arrow/.style={-Latex, thick}
]
\node[stage] (if) {IF\\取指};
\node[reg, right=0.5cm of if] (ifid) {IF/ID};
\node[stage, right=0.5cm of ifid] (id) {ID\\译码};
\node[reg, right=0.5cm of id] (idex) {ID/EX};
\node[stage, right=0.5cm of idex] (ex) {EX\\执行};
\node[reg, right=0.5cm of ex] (exmem) {EX/MEM};
\node[stage, right=0.5cm of exmem] (mem) {MEM\\访存};
\node[reg, right=0.5cm of mem] (memwb) {MEM/WB};
\node[stage, right=0.5cm of memwb] (wb) {WB\\写回};
...
\node[stage, below=1.0cm of id] (control) {Control\\IMMGEN\\RegFile};
\node[stage, below=1.0cm of ex] (hazard) {Forwarding\\Hazard Detection};
\draw[arrow] (control) -- (id);
\draw[arrow] (hazard) -- (ex);
\draw[arrow, dashed] (wb.south) |- (control.east);
\end{tikzpicture}
\caption{五级流水线CPU整体架构示意图}
\end{figure}
...
\scriptsize
\begin{tabularx}{\textwidth}{p{2.1cm}p{1.8cm}p{1.8cm}p{1.5cm}p{1.5cm}p{2.2cm}p{1.6cm}X}
\toprule
指令类型 & 立即数类型 & PC来源 & ALUSrcA & ALUSrcB & ALU功能 & 写回来源 & 说明 \\
\midrule
R型 & 无 & PC+4 & rs1 & rs2 & funct决定 & ALU & 寄存器-寄存器运算 \\
I型运算 & I型 & PC+4 & rs1 & imm & funct决定 & ALU & 寄存器-立即数运算 \\
Load & I型 & PC+4 & rs1 & imm & 加法求地址 & MEM & 从DRAM/外设读取数据 \\
Store & S型 & PC+4 & rs1 & imm & 加法求地址 & 无 & 写入DRAM/外设 \\
Branch & B型 & 条件分支 & rs1 & rs2 & 比较 & 无 & 分支成立时PC=PC+imm \\
\code{lui} & U型 & PC+4 & 无 & imm & 无 & IMM & 立即数写回rd \\
\code{auipc} & U型 & PC+4 & PC & imm & 加法 & ALU & PC+imm写回rd \\
\code{jal} & J型 & PC+imm & 无 & 无 & 无 & PC+4 & 跳转并写回返回地址 \\
\code{jalr} & I型 & rs1+imm & rs1 & imm & 加法 & PC+4 & 间接跳转并写回返回地址 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{企业工程模板接口适配}

本设计严格采用企业模板要求的\code{myCPU}接口，直接连接IROM、DRAM和外设访问端口。CPU核心内部不直接固定LED和数码管逻辑，而是通过统一的\code{perip}接口输出地址、写使能、写数据和访存宽度，由外部SoC或模板中的外设桥完成地址译码。

\subsection{统一外设访问机制}

CPU使用与load/store指令一致的数据通路访问DRAM和外设。ALU在EX阶段计算访存地址，MEM阶段将地址输出到\code{perip\_addr}，同时将写使能、写数据和访问宽度输出到外设接口。这种设计保持了CPU核心的通用性，便于后续扩展更多MMIO外设。

\subsection{调试跟踪接口预留}

在\code{ENABLE\_DEBUG\_TRACE}宏开启时，CPU会输出写回阶段相关调试信息，包括\code{debug\_wb\_have\_inst}、\code{debug\_wb\_pc}、\code{debug\_wb\_ena}、\code{debug\_wb\_reg}和\code{debug\_wb\_value}等，便于仿真阶段对指令提交结果进行追踪与比对。
...
\begin{enumerate}
    ...
    \item 第十届全国大学生集成电路创新创业大赛``竞业达''企业命题初赛、分赛区决赛比赛要求补充说明。
    \item \todo{补充Vivado官方文档、课程资料或其他参考资料。}
\end{enumerate}
